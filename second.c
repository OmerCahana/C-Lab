...
void updateDataLabelsAddress(int IC)
{
	...
	{
		if (g_labelArr[i].type != CMD)
		{
			if((g_entryLines[0] != NULL)&& (strcmp(g_labelArr[i].name,g_entryLines[0]->lineStr) != 0))
			/* Increase the adress */
			{
...
int getOpTypeId(operandInfo op)
{
	...
	if (op.type != INVALID)
	{
		/* NUMBER = 0, LABEL = 1, LIST =2 ,REGISTER = 3 */
		return (int)op.type;
	}
