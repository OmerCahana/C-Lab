...
bool addNumberToData(int num, int *IC, int *DC)
{
...
bool addStringToData(char *str, int *IC, int *DC, int lineNum)
{
	do
	{
		if (!addNumberToData((int)*str, IC, DC))
		{
			return FALSE;
		}
	} while (*str++);
...
void parseDataDirc(lineInfo *line, int *IC, int *DC)
{
	...
	FOREVER
	{
		...
		{
			if (!addNumberToData(operandValue, IC, DC))
			{
				/* Not enough memory */
				line->isError = TRUE;
				return;
